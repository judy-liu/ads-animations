...
function getBouncyEasing(bounciness: number): [number, number, number, number] {
    ...
    // Bouncy easing: [0.68, -0.55, 0.265, 1.55] (elastic)

    const smoothEasing: [number, number, number, number] = [0.25, 0.1, 0.25, 1];
    const bouncyEasing: [number, number, number, number] = [0.68, -0.55, 0.265, 1.55];

    // Interpolate between the two easings
    const interpolated: [number, number, number, number] = [
        smoothEasing[0] + (bouncyEasing[0] - smoothEasing[0]) * (bounciness / 100),
        smoothEasing[1] + (bouncyEasing[1] - smoothEasing[1]) * (bounciness / 100),
        smoothEasing[2] + (bouncyEasing[2] - smoothEasing[2]) * (bounciness / 100),
        smoothEasing[3] + (bouncyEasing[3] - smoothEasing[3]) * (bounciness / 100),
    ];
...
function Cursor({ x, y, bounciness }: CursorProps) {
    return (
        <motion.div
            style={{
                position: "absolute",
            ...
            animate={{ x, y }}
            transition={{ duration: 1.2, ease: getBouncyEasing(bounciness) }}
        >
    ...
    return (
        ...
        >
            ...
            >
                <g>
                    <path
                        d="M2.08487 2.08487L14.5941 14.5941"
                        stroke={getThemedColor("#6b778c", "#9fadbc", themeMode)}
                        strokeWidth="2.08487"
                        strokeLinecap="round"
                    />
                    <path
                        d="M14.5941 2.08487L2.08487 14.5941"
                        stroke={getThemedColor("#6b778c", "#9fadbc", themeMode)}
                        strokeWidth="2.08487"
                        strokeLinecap="round"
                    />
...
function Flag({
    ...
    return (
        <motion.div
            ...
            transition={{ duration: 0.6, ease: getBouncyEasing(bounciness) }}
        >
...
function SelectionFrame({
    ...
    return (
        <motion.div
            ...
            transition={{
                duration,
                ease: getBouncyEasing(bounciness),
            }}
        >
            ...
            {isVisible && (
                ...
                >
                    ...
                    <motion.div
                        ...
                        transition={{ duration: 0.2, ease: getBouncyEasing(bounciness) }}
                    >
                        {!showLoading && (
                            <div
                                style={{
...
export default function ModalInteractionAdjustable({
    ...
    return (
        ...
        >
            ...
            >
                <Flag shouldMove={modalShouldMove} themeMode={themeMode} bounciness={bounciness} />
                <SelectionFrame
                ...
                <WhiteModal
                    ...
                    bounciness={bounciness}
                />
                <Cursor x={cursorPosition.x} y={cursorPosition.y} bounciness={bounciness} />
            </motion.div>
        </div>
    );
}
